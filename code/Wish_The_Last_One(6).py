...
def single_run_hybrid(m, k, T, reward_star, mu_on,gap_offline, N, mu_hat_off, V1):
    ...
    for t in range(1, T+1):
        print("--------------------------------------------------")
        save_path = r"\home\h\桌面\test\good"
...
if __name__ == "__main__":
    ...
    reward_off = 0
    reward = []
    num_each_arm = int(input("num_each_arm:"))
    ...
    plt.grid(True, linestyle='--', alpha=0.7)
    file_name = f"V={bias},N={num_each_arm}.pdf"  # Save as PDF
    plt.savefig(os.path.join(save_path, file_name), format='pdf')
    plt.show()
